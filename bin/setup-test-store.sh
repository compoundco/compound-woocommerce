...
# Storefront products. Each entry: sku|name|price|scientific description|demo lot. The SKUs match the Compound
...
for row in "${PRODUCTS[@]}"; do
  ...
  product_id="$(wp wc product create --user=admin --name="$name" --sku="$sku" --regular_price="$price" \
    ...
    --images="[{\"id\":$image_id}]" --meta_data="[{\"key\":\"_compound_coa_url\",\"value\":\"$coa_url\"},{\"key\":\"_compound_lot_number\",\"value\":\"$lot\"},{\"key\":\"_compound_lab_name\",\"value\":\"Demo Analytical Laboratory\"},{\"key\":\"_compound_analysis_type\",\"value\":\"HPLC identity and purity\"},{\"key\":\"_compound_coa_date\",\"value\":\"2026-08-01\"}]" --porcelain)"
  echo "    $sku  $name  \$$price"
done

echo "==> WooCommerce: theme + branding (Storefront)"
wp theme install storefront --activate >/dev/null 2>&1 || true
wp option update blogname "Chefs Peps" >/dev/null 2>&1
